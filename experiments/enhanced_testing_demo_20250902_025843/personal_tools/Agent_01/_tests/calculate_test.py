...
def run_tests():
    ...
    try:
        ...
        if output == expected:
            record_result("Large number multiplication", True)
        else:
            record_result("Large number multiplication", False, f"Expected {expected}, got {output}")
    except Exception as e:
        record_result("Large number multiplication", False, str(e))
    ...
    try:
        output = calculate.execute("add", 5)
    ...
    # 7. Negative numbers
    try:
        output = calculate.execute("subtract", -5, -3)
        if output == -2:
            record_result